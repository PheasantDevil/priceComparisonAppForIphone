...
resource "aws_api_gateway_integration" "lambda" {
  ...
  type                    = "AWS_PROXY"
  uri                     = try(data.aws_lambda_function.existing_get_prices[0].invoke_arn, aws_lambda_function.get_prices[0].invoke_arn)
}
...
resource "aws_lambda_permission" "api_gateway" {
  count = try(data.aws_lambda_function.existing_get_prices[0].function_name, "") != "" ? 0 : 1

  statement_id  = "AllowAPIGatewayInvoke"
  action        = "lambda:InvokeFunction"
  function_name = try(data.aws_lambda_function.existing_get_prices[0].function_name, aws_lambda_function.get_prices[0].function_name)
