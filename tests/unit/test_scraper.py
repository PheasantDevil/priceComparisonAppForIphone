...
def test_parse_response(scraper):
    ...
    result = scraper.parse_response(soup)
    assert len(result) > 0
    assert all(isinstance(item, PriceData) for item in result)

def test_measure_request(scraper):
    start_time = datetime.now()
    result = scraper.measure_request(lambda: None)
    assert isinstance(result, float)
    assert result >= 0

def test_record_cache_hit(scraper):
    url = "https://example.com"
    scraper.record_cache_hit(url)
    assert url in scraper.cache_hits

def test_record_cache_miss(scraper):
    url = "https://example.com"
    scraper.record_cache_miss(url)
    assert url in scraper.cache_misses
